...
// Fetch all seats in this transaction
...
$seatsStmt = $pdo->prepare("
    ...
    AND customer_id = ?
");
$seatsStmt->execute([$transaction_uuid, currentUserId()]);
$allBookings = $seatsStmt->fetchAll();

$seatLabels = array_column($allBookings, 'seat_label');
$displaySeats = implode(', ', $seatLabels);
...
$totalAmountFromPayment = $payStmt->fetchColumn();

if (!$totalAmountFromPayment) {
    $totalAmount = array_sum(array_column($allBookings, 'amount'));
} else {
    $totalAmount = $totalAmountFromPayment;
}

$realAmount = $totalAmount;
$displayAmount = number_format((float)$realAmount, 2, '.', '');

// Logic for eSewa Sandbox Testing
...
// Use the existing transaction_uuid from database if available
if (!$transaction_uuid) {
        ...
        <div style="padding: 32px;">
            <div style="background: #fcfdfe; border: 1px dashed #d1d9e2; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                    ...
                    <span style="color: #1a1f36; font-weight: 600; font-size: 14px;"><?= htmlspecialchars($displaySeats) ?></span>
                </div>
                <div style="border-top: 1px solid #eef2f7; margin-top: 12px; padding-top: 12px; display: flex; justify-content: space-between;">
                    ...
                    <span style="color: #0b5cff; font-weight: 700; font-size: 18px;">NPR <?= htmlspecialchars($displayAmount) ?></span>
                </div>
                <?php if (isset($ESEWA_TEST_MODE) && $ESEWA_TEST_MODE): ?>
                <div style="margin-top: 10px; padding: 8px; background: #fff4e5; border-radius: 6px; border: 1px solid #ffd599; color: #663c00; font-size: 12px; text-align: center;">
                    <strong>Sandbox Mode:</strong> You will be charged Rs. <?= $payment_amount ?> for testing.
                </div>
                <?php endif; ?>
            </div>
